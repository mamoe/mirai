...
internal class DirectMessageProcessor(
    ...
    override suspend fun NoticePipelineContext.processImpl(data: GuildMsg.MsgOnlinePush) {
        ...
        try {
            val item = data.msgs.first()

            val tinyId = item.head?.routingHead?.fromTinyId
            val directGuildId = item.head?.routingHead?.guildId
            val directChannelId = item.head?.routingHead?.channelId
            ...
            val isFromSelfAccount = (tinyId == bot.account.tinyId) || (fromUin == bot.id)
            val guild = bot.getGuildOrFail(directMessageMember.sourceGuildId) as GuildImpl? ?: return
            val sender = guild.members.find { it.id == tinyId } as GuildMemberImpl? ?: return
