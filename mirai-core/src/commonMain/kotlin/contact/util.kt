...
internal fun net.mamoe.mirai.event.events.MessageEvent.logMessageReceived() {
    ...
    fun renderDirectMessage(
        guild: Guild,
        senderName: String,
        sender: GuildMember,
        message: MessageChain
    ): String {
        return "[${guild.name}(${guild.id})] ${senderName}(${sender.id})[私信] -> ${
            renderMessage(
                message
            )
        }"
    ...
    fun renderDirectMessageSync(
        guild: Guild,
        sender: GuildMember,
        message: MessageChain
    ): String {
        return "[${guild.name}(${guild.id})] ${senderName}(${sender.id})[私信][SYNC] <- ${
            renderMessage(
                message
            )
        }"
