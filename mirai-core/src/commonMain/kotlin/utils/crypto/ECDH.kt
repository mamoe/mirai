...
internal interface ECDHKeyPair {
     ...
     */
    val mockedShareKey: ByteArray

    /**
     * 私匙和动态公匙([ECDHInitialPublicKey]) 计算得到的 publicKey
     */
    val mockedPublicKey: ByteArray
    ...
    object DefaultStub : ECDHKeyPair {
        ...
        override val mockedShareKey: ByteArray get() = defaultShareKey
        override val mockedPublicKey: ByteArray
...
internal expect class ECDH(keyPair: ECDHKeyPair) {
    ...
    companion object {
         ...
         */
        fun verifyPublicKey(version: Int, rsaKey: String, publicKey: String, publicKeySign: String): Boolean
...
public object ECDHInitialPublicKeyUpdater {
    @Serializable
    ...
    public fun getLatestPublicKey(
        uin: Long,
        currentPublicKey: ECDHInitialPublicKey = defaultInitialPublicKey
    ): ECDHInitialPublicKey {

        return kotlin.runCatching {
            MiraiLogger.TopLevel.info("Start to check if the ecdh key still valid.")
            if (currentPublicKey.expireTime > currentTimeSeconds()) {
                MiraiLogger.TopLevel.info("ECDH key is valid.")
                return currentPublicKey
            } else {
                runBlocking {
                    MiraiLogger.TopLevel.info("ECDH key is invalid, start to fetch ecdh public key from server.")
                    val respStr =
                        Mirai.Http.get<String>("https://keyrotate.qq.com/rotate_key?cipher_suite_ver=305&uin=$uin")
                    val resp = JsonForCache.decodeFromString(Response.serializer(), respStr)
                    resp.pubKeyMeta.let { meta ->
                        val isValid = ECDH.verifyPublicKey(
                            version = meta.keyVer,
                            rsaKey = "",
                            publicKey = meta.pubKey,
                            publicKeySign = meta.pubKeySign
                        )
                        check(isValid) { "ECDH public key which from server is invalid" }
                        MiraiLogger.TopLevel.info("Successfully fetch ecdh public key from server.")
                        ECDHInitialPublicKey(meta.keyVer, meta.pubKey, currentTimeSeconds() + resp.querySpan)
                    }
                }
            }
        }.getOrElse {
            MiraiLogger.TopLevel.error("Failed to fetch ecdh public key from server, use default key instead", it)
